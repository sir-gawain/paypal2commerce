...
const PAYERR_CURL_CONN = 0x66661;
const PAYERR_PAYPAL_SV = 0x66662;
const PAYERR_WRONG_CURRENCY = 0x66663;
const PAYERR_AMOUNT_MISMATCH = 0x66664;
const PAYERR_WRONG_COUNTRY = 0x66665;

class tx_paypal2commerce extends \CommerceTeam\Commerce\Payment\PaymentAbstract {
    ...
    function checkCurlProxy(&$cURLHandle) {
        if ($GLOBALS['TYPO3_CONF_VARS']['SYS']['curlUse']
            ...
            if (isset($GLOBALS['TYPO3_CONF_VARS']['SYS']['curlProxyUserPass'])
                ...
                curl_setopt($cURLHandle, CURLOPT_PROXYUSERPWD, $GLOBALS['TYPO3_CONF_VARS']['SYS']['curlProxyUserPass']);
            }
        }
    }
    ...
    function checkFromPaypal() {
        $token = urlencode(GeneralUtility::_GP('token'));
        $paymentAmount = urlencode(GeneralUtility::_GP('paymentAmount'));
        $paymentType = urlencode(GeneralUtility::_GP('paymentType'));
        $currCodeType = urlencode(GeneralUtility::_GP('currencyCodeType'));
        $payerID = urlencode(GeneralUtility::_GP('PayerID'));
     ...
     * @param  \CommerceTeam\Commerce\Domain\Model\Basket $basket the basket object
     * @return boolean                    true or false
     */
    function finishingFunction(
        array $config = array(),
        array $session = array(),
        \CommerceTeam\Commerce\Domain\Model\Basket $basket = null
    ) {
        // ok, we get the form
        if ('' != GeneralUtility::_POST('tx_commerce_pi3')) {
            $GLOBALS['TSFE']->fe_user->setKey('ses', 'comment', GeneralUtility::removeXSS(strip_tags($this->parentObject->piVars['comment'])));
            $GLOBALS["TSFE"]->storeSessionData();
            $this->sendToPaypal($basket->getSumGross(), $this->parentObject->conf['currency']);
        }
        if (!$this->checkFromPaypal()) {
            ...
            $this->parentObject->piVars['comment'] = $this->comment;
    ...
    function getFromPaypal() {
        ...
        try {
            ...
            if (!$this->amountEqual($basket->getSumGross(), $_REQUEST['paymentAmount'] * 100)) {
                // wrong sum
            ...
            if ($ack == "SUCCESS") {
                ...
                if ($this->amountEqual($basket->getSumGross(), $resArray['AMT'] * 100)) {
     ...
     */
    function getSpecialFinishingForm($config, $session, $basket) {
     ...
     */
    function hasSpecialFinishingForm($request) {
     ...
     */
    function proofData(array $formData = array()) {
        return true;
    }
    ...
    function sendToPaypal($amount, $currencyCodeType = 'EUR') {
        print_r(array($amount, $currencyCodeType));
        try {
...
//            $baseurl .= '/';
        ...
        // SET Address
        $addresstyp = 'billing';
        if (is_array($this->parentObject->sessionData['delivery']) && sizeof($this->parentObject->sessionData['delivery']) > 0) {
            $addresstyp = 'delivery';
        }
        $addr = $this->parentObject->sessionData[$addresstyp];
     ...
     */
    function updateOrder($orderUid, array $session = array()) {
